...
def predict(sess, folder, image_file):
    """
    ...
    image, image_data = preprocess_image(
        folder + '/' + image_file, model_image_size=(608, 608))
...
def main():
    folder = "data"
    prefix = "cabf7be1-36a39a28"
    extension = ".jpg"
    for x in range(1, 10):
        ind = "{:07d}".format(x)
        filename = prefix + '-' + ind + extension
        print(filename)
        predict(sess, folder, filename)
